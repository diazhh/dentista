...
export default function TenantDetailPage() {
  ...
  const handleReactivate = async () => {
    if (!confirm('¿Estás seguro de reactivar este tenant?')) return;
    try {
      const token = localStorage.getItem('accessToken');
  ...
  const handleRemoveUser = async (userId: string, userName: string) => {
    if (!confirm(`¿Estás seguro de remover a ${userName} de este tenant?`)) return;
    try {
      const token = localStorage.getItem('accessToken');
      await axios.delete(
    ...
    <div className="space-y-6">
      ...
      <div className="border-b border-gray-200">
        <nav className="-mb-px flex space-x-8">
          ...
          >
            ...
            Usuarios ({tenant.memberships.length})
          </button>
          <button
      ...
      {activeTab === 'info' && (
        <div className="bg-white shadow rounded-lg p-6 space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-gray-900">Información del Tenant</h2>
            {!editMode ? (
              <button
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div>
              ...
              <p className="text-gray-900">{tenant.owner.name}</p>
              <p className="text-sm text-gray-500">{tenant.owner.email}</p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Fecha de Creación</label>
              <p className="text-gray-900">{new Date(tenant.createdAt).toLocaleDateString('es-ES')}</p>
            </div>
          </div>

          <div className="border-t pt-6">
            <h3 className="text-md font-semibold text-gray-900 mb-4">Estadísticas</h3>
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-gray-50 p-4 rounded-lg">
                <p className="text-sm text-gray-600">Citas</p>
                <p className="text-2xl font-bold text-gray-900">{tenant._count.appointments}</p>
              </div>
              <div className="bg-gray-50 p-4 rounded-lg">
                <p className="text-sm text-gray-600">Usuarios</p>
                <p className="text-2xl font-bold text-gray-900">{tenant._count.memberships}</p>
              </div>
              <div className="bg-gray-50 p-4 rounded-lg">
                <p className="text-sm text-gray-600">Pacientes</p>
                <p className="text-2xl font-bold text-gray-900">{tenant._count.patientDentistRelations}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white shadow rounded-lg p-6">
          ...
          {usersLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="mt-2 text-sm text-gray-500">No hay usuarios en este tenant</p>
            </div>
          ) : (
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="STAFF_BILLING">Facturación</option>
        ...
        <div className="bg-white shadow rounded-lg p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-6">
            <div>
              ...
              <p className="text-gray-900 font-semibold">{tenant.subscriptionTier}</p>
            </div>
            <div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Máximo de Pacientes</label>
              <p className="text-gray-900">{tenant.maxPatients}</p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Storage (GB)</label>
              <p className="text-gray-900">{tenant.storageGB} GB</p>
            </div>
          </div>
        </div>
      )}
