  ...
  return (
    <div className="p-3 sm:p-6">
      ...
      <div className="mb-4 sm:mb-6">
        <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900">Mis Facturas</h1>
        <p className="text-sm sm:text-base text-gray-600 mt-1">Consulta y paga tus facturas pendientes</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mb-4 sm:mb-6">
        ...
        <div className="bg-white rounded-lg shadow-sm p-3 sm:p-4 border border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs sm:text-sm text-gray-500">Pagado este mes</p>
              <p className="text-lg sm:text-xl font-bold text-gray-900">
                {formatCurrency(
                  invoices
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        {invoices.length === 0 ? (
          <div className="text-center py-10 sm:py-12">
            ...
            <p className="text-sm sm:text-base text-gray-500">No tienes facturas</p>
          </div>
        ) : (
          <>
            {/* Desktop Table View */}
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="mt-2 text-sm text-gray-500 space-y-1">
                          <p>Fecha de emision: {formatDate(invoice.issueDate)}</p>
                        ...
                        <div className="mt-3">
                          <p className="text-sm text-gray-600">
                            ...
                            {invoice.items.length > 2 && ` y ${invoice.items.length - 2} mas...`}
                          </p>
                        </div>
                      </div>
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between gap-2 mb-3">
                      <div>
                        ...
                        <span
                          className={`inline-flex items-center gap-1 px-2 py-0.5 text-[10px] sm:text-xs font-medium rounded-full mt-1 ${status.color}`}
                        >
                          <StatusIcon className="w-3 h-3" />
                          {status.label}
                        </span>
                    ...
                    <div className="text-xs sm:text-sm text-gray-500 space-y-1 mb-3">
                      <p>Emision: {formatDateShort(invoice.issueDate)}</p>
                      <p>Vence: {formatDateShort(invoice.dueDate)}</p>
                    </div>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      <button
                        onClick={() => handleDownloadPdf(invoice.id)}
                        className="flex items-center gap-1.5 px-3 py-1.5 text-gray-600 hover:text-gray-800 hover:bg-gray-100 rounded-lg transition-colors text-xs sm:text-sm"
                      >
                      ...
                      {(invoice.status === 'SENT' || invoice.status === 'OVERDUE') &&
                        invoice.balance > 0 && (
                          <button
                            onClick={() => handlePayNow(invoice)}
                            className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 text-white text-xs sm:text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors"
                          >
                            <CreditCard className="w-4 h-4" />
                            Pagar ahora
                          </button>
          ...
          <div className="bg-white rounded-xl sm:rounded-2xl shadow-xl w-full max-w-md">
            <div className="p-4 sm:p-6">
              <div className="flex items-center justify-between mb-3 sm:mb-4">
                <h2 className="text-lg sm:text-xl font-semibold text-gray-900">Pagar Factura</h2>
                <button
              ...
              <div className="space-y-3 sm:space-y-4">
                ...
                <div className="bg-gray-50 rounded-lg p-3 sm:p-4">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-xs sm:text-sm text-gray-600">Factura</span>
                    <span className="text-sm sm:text-base font-medium">#{selectedInvoice.invoiceNumber}</span>
                  </div>
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-xs sm:text-sm text-gray-600">Subtotal</span>
                    <span className="text-sm sm:text-base">{formatCurrency(selectedInvoice.subtotal)}</span>
                  </div>
                  {selectedInvoice.tax > 0 && (
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-xs sm:text-sm text-gray-600">Impuestos</span>
                      <span className="text-sm sm:text-base">{formatCurrency(selectedInvoice.tax)}</span>
                    </div>
                  )}
                  {selectedInvoice.discount > 0 && (
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-xs sm:text-sm text-gray-600">Descuento</span>
                  ...
                  {selectedInvoice.amountPaid > 0 && (
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-xs sm:text-sm text-gray-600">Ya pagado</span>
                      <span className="text-sm sm:text-base text-green-600">
                  ...
                  <div className="border-t border-gray-200 pt-2 mt-2">
                    <div className="flex justify-between items-center">
                      <span className="text-sm sm:text-base font-semibold text-gray-900">A pagar</span>
                ...
                <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                  <button
                    onClick={() => setShowPaymentModal(false)}
                    className="flex-1 px-3 sm:px-4 py-2.5 sm:py-3 border border-gray-300 text-gray-700 text-sm sm:text-base font-medium rounded-lg hover:bg-gray-50 transition-colors order-2 sm:order-1"
                  >
                    Cancelar
                  </button>
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 px-3 sm:px-4 py-2.5 sm:py-3 bg-blue-600 text-white text-sm sm:text-base font-medium rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 order-1 sm:order-2"
                  >
                    {paymentLoading ? (
                      <>
